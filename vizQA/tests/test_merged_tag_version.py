"""Tests for :mod:`vizQA.merged_tag_version`."""

from unittest.mock import MagicMock, patch

from packaging.version import parse as parse_version

from vizQA.merged_tag_version import max_merged_tag_version


def test_git_failure_returns_default():
    proc = MagicMock(returncode=1, stdout="")
    with patch("vizQA.merged_tag_version.subprocess.run", return_value=proc):
...
def test_passes_git_cwd():
    ...
    with patch("vizQA.merged_tag_version.subprocess.run", return_value=proc) as run:
